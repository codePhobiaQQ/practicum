	...
	if ( $is_interactive ) {
		...
		wp_register_script_module(
			...
			array(
				...
				array(
					'id'     => '@wordpress/interactivity-roomsRoutes',
					'import' => 'dynamic',
				),
			),
		...
		if ( $p->next_tag() ) {
			...
			$p->set_attribute( 'data-wp-roomsRoutes-region', 'query-' . $attributes['queryId'] );
		...
		if ( ! isset( $render_query_callback ) ) {
			...
			$render_query_callback = static function ( $content, $block ) use ( &$enhanced_query_stack, &$dirty_enhanced_queries, &$render_query_callback ) {
				...
				if ( isset( $dirty_enhanced_queries[ $block['attrs']['queryId'] ] ) ) {
					// Disable navigation in the roomsRoutes store config.
					wp_interactivity_config( 'core/roomsRoutes', array( 'clientNavigationDisabled' => true ) );
