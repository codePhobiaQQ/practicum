...
final class WP_Interactivity_API {
	...
	private static $directive_processors = array(
		...
		'data-wp-roomsRoutes-region' => 'data_wp_router_region_processor',
	...
	/**
	 * Flag that indicates whether the `data-wp-roomsRoutes-region` directive has
	 ...
	 * `data-wp-roomsRoutes-region` directive or not.
	 *
	 * @since 6.5.0
	 * @var bool
	 */
	...
	public function register_script_modules() {
		...
		wp_register_script_module(
			'@wordpress/interactivity-roomsRoutes',
			includes_url( "js/dist/interactivity-roomsRoutes$suffix.js" ),
	...
	/**
	 * Returns the CSS styles for animating the top loading bar in the roomsRoutes.
	 *
	 * @since 6.5.0
	 *
	 * @return string The CSS styles for the roomsRoutes's top loading bar animation.
	 */
	private function get_router_animation_styles(): string {
		return <<<CSS
			.wp-interactivity-roomsRoutes-loading-bar {
				position: fixed;
				top: 0;
				left: 0;
				margin: 0;
			...
			.wp-interactivity-roomsRoutes-loading-bar.start-animation {
				animation: wp-interactivity-roomsRoutes-loading-bar-start-animation 30s cubic-bezier(0.03, 0.5, 0, 1) forwards
			}
			.wp-interactivity-roomsRoutes-loading-bar.finish-animation {
				animation: wp-interactivity-roomsRoutes-loading-bar-finish-animation 300ms ease-in
			}
			@keyframes wp-interactivity-roomsRoutes-loading-bar-start-animation {
				0% { transform: scaleX(0); transform-origin: 0 0; opacity: 1 }
				100% { transform: scaleX(1); transform-origin: 0 0; opacity: 1 }
			}
			@keyframes wp-interactivity-roomsRoutes-loading-bar-finish-animation {
				0% { opacity: 1 }
				50% { opacity: 1 }
				100% { opacity: 0 }
			}
CSS;
	...
	public function print_router_loading_and_screen_reader_markup() {
		echo <<<HTML
			<div
				class="wp-interactivity-roomsRoutes-loading-bar"
				data-wp-interactive="core/roomsRoutes"
...
HTML;
	...
	/**
	 * Processes the `data-wp-roomsRoutes-region` directive.
	 *
	...
	private function data_wp_router_region_processor( WP_Interactivity_API_Directives_Processor $p, string $mode ) {
		if ( 'enter' === $mode && ! $this->has_processed_router_region ) {
			...
			// Initialize the `core/roomsRoutes` store.
			$this->state(
				'core/roomsRoutes',
				array(
					'navigation' => array(
			...
			wp_register_style( 'wp-interactivity-roomsRoutes-animations', false );
			wp_add_inline_style( 'wp-interactivity-roomsRoutes-animations', $this->get_router_animation_styles() );
			wp_enqueue_style( 'wp-interactivity-roomsRoutes-animations' );
